...
class DistanceToWeekDayInput(BaseModel):
    date: str = Field(
        description="The date to calculate the distance from, in the format YYYY-MM-DD"
    )
    weekday: int = Field(
        description="The weekday to calculate the distance to. 0 is Monday, 6 is Sunday."
    )


@tool(args_schema=DistanceToWeekDayInput)
def distance_to_week_day(date: str, weekday: int) -> int:
    """Calculate the number of days until the given weekday"""  # noqa E501
    the_date = datetime.strptime(date, "%Y-%m-%d")
    d = (weekday - the_date.weekday()) % 7
